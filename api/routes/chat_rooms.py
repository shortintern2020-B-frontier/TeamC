...
async def get_chat_room(id: int, database: Database = Depends(get_connection)):
    select_room_query = f"select * from chat_rooms left join user_chat_rooms on chat_rooms.id = user_chat_rooms.chat_room_id where user_chat_rooms.user_id = {id} and user_chat_rooms.valid = 1"
